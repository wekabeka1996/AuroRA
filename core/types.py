...
class MarketSnapshot:
    ...
    def __post_init__(self) -> None:
        ...
        if any(v < 0 for v in self.bid_volumes_l) or any(
            v < 0 for v in self.ask_volumes_l
        ):
    ...
    def microprice(self, levels: int = 1) -> Decimal:
        if levels <= 1:
            ...
            return (
                self.mid
                if denom == 0
                else (a1 * self.bid_price + b1 * self.ask_price) / denom
            )
        ...
        return (
            self.mid
            if denom == 0
            else (a * self.bid_price + b * self.ask_price) / denom
        )
...
class XAIRecord:
    ...
    def to_json(self) -> str:
        ...
        return json.dumps(
            self, default=_conv, ensure_ascii=False, separators=(",", ":")
        )
...
def latency_degradation(
    edge0_bps: float, kappa_bps_per_ms: float, latency_ms: float
) -> float:
...
def _test_edge_math() -> None:
    ...
    e = expected_pnl(
        p=Decimal("0.62"), G=Decimal("8.0"), L=Decimal("6.0"), c=Decimal("2.0")
    )
