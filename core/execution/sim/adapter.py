...
class SimAdapter:
    ...
    def __init__(self, cfg: Optional[Dict[str, Any]] = None):
        ...
        self._obi_bias = _to_float(
            self.cfg.get("mock_obi_bias", os.getenv("AURORA_SIM_OBI_BIAS", 0.0)), 0.0
        )
        self._tfi_bias = _to_float(
            self.cfg.get("mock_tfi_bias", os.getenv("AURORA_SIM_TFI_BIAS", 0.0)), 0.0
        )
    ...
    def place_order(self, side: str, qty: float, price: Optional[float] = None):
        ...
        oid = self._sink.submit(
            order, market={"best_bid": None, "best_ask": None, "liquidity": {}}
        )
