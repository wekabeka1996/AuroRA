...
class SimLocalSink:
    ...
    def submit(
        self, order: Dict[str, Any], market: Optional[Dict[str, Any]] = None
    ) -> str:
        oid = (
            order.get("order_id")
            or f"sim-{int(self._time())}-{self.rng.randint(0, 9999)}"
        )
        o = dict(order)
        o["order_id"] = oid
    ...
    def on_tick(self, market_snapshot: Dict[str, Any]) -> None:
        ...
        for oid, o in list(self._orders.items()):
            ...
            traded = market_snapshot.get("traded_since_last", {}).get(
                o.get("price"), 0.0
            )
            eps = self.maker_eps
            ...
            evt = {
                ...
                "maker_queue_pos": 0,
                "fill_qty_step": fill_qty,
                "fill_ratio": (o.get("orig_qty") - o.get("remaining", 0.0))
                / max(1e-9, o.get("orig_qty")),
                "slip_bps": None,
                "ttl_ms": self.ttl_ms,
            }
            self._emit_seed_if_needed(evt)
