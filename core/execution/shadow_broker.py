...
warnings.warn(
    "core.execution.shadow_broker is archived; use adapters and router_v2",
    DeprecationWarning,
    stacklevel=2,
)
# -*- coding: utf-8 -*-
...
class ShadowBroker:
        ...
        try:
            ...
            for symbol_info in exchange_info.get("symbols", []):
                ...
                self.filters[symbol] = BinanceFilters(
                    ...
                    price_filter_max_price=Decimal(
                        price_filter.get("maxPrice", "999999999")
                    ),
                    price_filter_tick_size=Decimal(
                    ...
                    percent_price_multiplier_up=Decimal(
                        percent_price.get("multiplierUp", "1.1")
                    ),
                    percent_price_multiplier_down=Decimal(
                        percent_price.get("multiplierDown", "0.9")
                    ),
                )
    ...
    def _validate_order(
        ...
        if quantity < filters.lot_size_min_qty:
            return OrderReject(
                "LOT_SIZE", f"Quantity {quantity} < min {filters.lot_size_min_qty}"
            )

        if quantity > filters.lot_size_max_qty:
            return OrderReject(
                "LOT_SIZE", f"Quantity {quantity} > max {filters.lot_size_max_qty}"
            )
        ...
        if remainder != 0:
            return OrderReject(
                "LOT_SIZE",
                f"Quantity {quantity} not multiple of step size {filters.lot_size_step_size}",
            )
        ...
        if price is not None:
            if price < filters.price_filter_min_price:
                return OrderReject(
                    "PRICE_FILTER",
                    f"Price {price} < min {filters.price_filter_min_price}",
                )

            if price > filters.price_filter_max_price:
                return OrderReject(
                    "PRICE_FILTER",
                    f"Price {price} > max {filters.price_filter_max_price}",
                )
            ...
            if remainder != 0:
                return OrderReject(
                    "PRICE_FILTER",
                    f"Price {price} not multiple of tick size {filters.price_filter_tick_size}",
                )

        # Validate notional
        notional_price = (
            price if price is not None else Decimal("50000")
        )  # Estimate for market orders
        notional = quantity * notional_price

        if notional < filters.min_notional:
            return OrderReject(
                "MIN_NOTIONAL", f"Notional {notional} < min {filters.min_notional}"
            )
    ...
    def submit_order(
        ...
        rejection = self._validate_order(
            symbol, side, order_type, qty_rounded, price_rounded
        )
        if rejection:
            # Log rejection event
        ...
        self._log_order_event(
            "FILLED", symbol, side, order_type, qty_rounded, price_rounded, "SUCCESS"
        )
    ...
    def validate_and_round_order(
        ...
        qty_rounded = self._round_quantity(symbol, qty_decimal)
        price_rounded = (
            self._round_price(symbol, price_decimal) if price_decimal else None
        )
        ...
        if rejection:
            return (
                False,
                f"{rejection.reason}: {rejection.details}",
                float(qty_rounded),
                float(price_rounded) if price_rounded else None,
            )
