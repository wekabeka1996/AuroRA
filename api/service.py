...
async def lifespan(app: FastAPI):
    ...
    try:
        ov_path = ((cfg or {}).get("overlays") or {}).get(
    ...
    try:
        ...
        if isinstance(aurora_mode, str) and aurora_mode.lower() == "shadow":
            raise RuntimeError(
                "'shadow' runtime mode is removed; use AURORA_MODE=live or testnet"
            )
    ...
    # Event emitter
    try:
        emitter_path = ((cfg or {}).get("logging") or {}).get(
    ...
    try:
        cfg_name = (
            os.getenv("AURORA_CONFIG")
            or os.getenv("AURORA_CONFIG_NAME")
            or "default(v4_min.yaml)"
        )
    ...
    try:
        ...
        ack_tracker = AckTracker(
            events_emit=lambda code, d: em_logger.emit(code, d), ttl_s=ack_ttl
        )
    ...
    try:
        ...
        app.state.order_loggers = OrderLoggers(
            success_path=(
                getattr(app.state, "session_dir", Path("logs")) / "orders_success.jsonl"
            ),
            failed_path=(
                getattr(app.state, "session_dir", Path("logs")) / "orders_failed.jsonl"
            ),
            denied_path=(
                getattr(app.state, "session_dir", Path("logs")) / "orders_denied.jsonl"
            ),
        )
    except Exception:
    ...
    try:
        ...
        l_guard_ms = float(
            guard_cfg.get("latency_guard_ms", os.getenv("AURORA_LATENCY_GUARD_MS", 30))
        )
        l_window_sec = int(
            guard_cfg.get(
                "latency_window_sec", os.getenv("AURORA_LATENCY_WINDOW_SEC", 60)
            )
        )
        l_cooloff = int(
            guard_cfg.get("cooloff_base_sec", os.getenv("AURORA_COOLOFF_SEC", 120))
        )
        l_halt_rep = int(
            guard_cfg.get(
                "halt_threshold_repeats", os.getenv("AURORA_HALT_THRESHOLD_REPEATS", 2)
            )
        )
    except Exception:
    ...
    try:
        ...
        if min_open_interval_ms is None:
            try:
                min_open_interval_ms = int(
                    os.getenv("AURORA_MIN_OPEN_INTERVAL_MS", "0")
                )
            except Exception:
        ...
        if cooldown_after_close_ms is None:
            try:
                cooldown_after_close_ms = int(
                    os.getenv("AURORA_COOLDOWN_AFTER_CLOSE_MS", "0")
                )
            except Exception:
...
LATENCY = Histogram(
    ...
    buckets=(1, 2, 5, 7.5, 10, 15, 25, 50, 75, 100, 150, 250),
...
REGIME = Gauge(
    "aurora_regime", "Current detected market regime", registry=METRICS_REGISTRY
)
REQUESTS = Counter(
...
OPS_TOKEN_ROTATIONS = Counter(
    "aurora_ops_token_rotations_total",
    "Total OPS token rotations",
    registry=METRICS_REGISTRY,
)
EVENTS_EMITTED = Counter(
    "aurora_events_emitted_total",
    "Total Aurora events emitted",
    ["code"],
    registry=METRICS_REGISTRY,
)
ORDERS_SUCCESS = Counter(
    "aurora_orders_success_total",
    "Total successful orders recorded",
    registry=METRICS_REGISTRY,
)
ORDERS_DENIED = Counter(
    "aurora_orders_denied_total",
    "Total denied orders recorded",
    registry=METRICS_REGISTRY,
)
ORDERS_REJECTED = Counter(
    "aurora_orders_rejected_total",
    "Total rejected orders recorded",
    registry=METRICS_REGISTRY,
)
OPS_AUTH_FAIL = Counter(
    "aurora_ops_auth_fail_total",
    "Total failed OPS auth attempts",
    registry=METRICS_REGISTRY,
)
...
DENY_RATE_15M = Gauge(
    ...
    "Deny rate over 15 minutes window",
    registry=METRICS_REGISTRY,
)
LATENCY_P99_MS = Gauge(
    "aurora_latency_p99_ms",
    "p99 latency in ms over recent window",
    registry=METRICS_REGISTRY,
)
ECE_GAUGE = Gauge("aurora_ece", "Expected Calibration Error", registry=METRICS_REGISTRY)
CVAR95_MIN = Gauge(
    "aurora_cvar95_min", "CVaR 95% min over recent window", registry=METRICS_REGISTRY
)
SSE_CLIENTS = Gauge(
    "aurora_sse_clients", "Current number of SSE clients", registry=METRICS_REGISTRY
)
SSE_DISCONNECTS = Counter(
    "aurora_sse_disconnects_total",
    "Total SSE client disconnects",
    registry=METRICS_REGISTRY,
)

PARENT_GATE_ALLOW = Counter(
    "aurora_parent_gate_allow_total", "Parent gate allows", registry=METRICS_REGISTRY
)
PARENT_GATE_DENY = Counter(
    "aurora_parent_gate_deny_total", "Parent gate denies", registry=METRICS_REGISTRY
)
EXPECTED_NET_REWARD_BLOCKED = Counter(
    "aurora_expected_net_reward_blocked_total",
    "Expected net reward blocks",
    registry=METRICS_REGISTRY,
)
ORDERS_SUBMITTED_SHADOW = Counter(
    "aurora_orders_submitted_shadow_total",
    "Shadow orders submitted total",
    registry=METRICS_REGISTRY,
)

# Model neutral streak per symbol (1 if last 200 p_cal values == 0.5)
MODEL_NEUTRAL_STREAK = Gauge(
    "aurora_model_neutral_streak",
    "Neutral p_cal streak",
    ["symbol"],
    registry=METRICS_REGISTRY,
)
...
class RateLimiter:
    ...
    def allow(self, ip: str, mutating: bool, now: float | None = None) -> bool:
        ...
        rec = self.state.setdefault(
            ip, {"tokens_g": self.rps_g, "tokens_m": self.rps_m, "ts": now}
        )
...
async def security_and_rl_middleware(request: Request, call_next):
    ...
    if mutating:
        if not (
            path.startswith("/ops")
            or path.startswith("/pretrade")
            or path.startswith("/posttrade")
        ):
...
async def predict(request: PredictionRequest):
    try:
        ...
        if ts is None:
            raise HTTPException(
                status_code=503, detail="Trading system not initialized"
            )

        result = await asyncio.wait_for(
            asyncio.to_thread(ts.predict, request.model_dump()),
            timeout=(
                (getattr(app.state, "cfg", {}) or {}).get("trading", {}) or {}
            ).get("max_latency_ms", 1000)
            / 1000.0,
        )
        REQUESTS.inc()
        ...
        slo = ((getattr(app.state, "cfg", {}) or {}).get("trading", {}) or {}).get(
            "max_latency_ms", 1000
        )
        raise HTTPException(
            status_code=503,
            detail=f"Prediction timeout: Service busy or latency SLO ({slo}ms) exceeded.",
        )
    except Exception as e:
...
async def health():
    ...
    order_profile = (cfg_all.get("pretrade", {}) or {}).get(
...
async def readiness(request: Request):
    """Readiness probe: always return a fixed-shape JSON body (even on 401/403/503)."""
    # Prepare readiness body first
    ts = getattr(app.state, "trading_system", None)
    model_loaded = bool(
        ts is not None
        and getattr(ts, "student", None) is not None
        and getattr(ts, "router", None) is not None
    )
    ...
    # Enforce OPS auth but keep structured error body on failure
    x_ops = request.headers.get("X-OPS-TOKEN")
    try:
        _ops_auth(x_ops_token=x_ops)
    except HTTPException as e:
        # Mirror auth status code but return readiness body in detail
        raise HTTPException(
    ...
    # Authorized: return 200 if ready, else 503 with body in detail
    if not model_loaded:
        raise HTTPException(
            status_code=503, detail=body, headers={"Cache-Control": "no-store"}
        )
    return JSONResponse(
        content=body, status_code=200, headers={"Cache-Control": "no-store"}
    )
...
async def pretrade_check(request: Request, req: PretradeCheckRequest):
    ...
    try:
        ...
        cd_until = (
            getattr(request.app.state, "_cooldown_until_ms", {}).get(sym)
            if sym
            else None
        )
        if (
            sym
            and isinstance(cd_until, (int, float))
            and now_ms < int(cd_until)
            and int(getattr(request.app.state, "cooldown_after_close_ms", 0)) > 0
        ):
            allow = False
            ...
            try:
                if emitter:
                    emitter.emit(
                        "COOLDOWN.ACTIVE",
                        {"symbol": sym, "ms_left": obs.get("cooldown_remaining_ms", 0)},
                    )
            except Exception:
                pass
        else:
            ...
            last_ok = (
                getattr(request.app.state, "_last_open_allow_ms", {}).get(sym)
                if sym
                else None
            )
            if (
                sym
                and min_iv > 0
                and isinstance(last_ok, (int, float))
                and (now_ms - int(last_ok)) < min_iv
            ):
                allow = False
                ...
                try:
                    if emitter:
                        emitter.emit(
                            ...
                            {
                                "symbol": sym,
                                "ms_left": obs.get("rate_limit_ms_left", 0),
                            },
                        )
                except Exception:
                    pass
                ...
                if sym and ps and isinstance(ps, dict):
                    ...
                    if open_side in {"buy", "sell"}:
                        ...
                        if side and (
                            (open_side == "buy" and side == "sell")
                            or (open_side == "sell" and side == "buy")
                        ):
                            ...
                            if min_hold > 0 and now_ms - since_ms < min_hold:
                                ...
                                try:
                                    if emitter:
                                        emitter.emit(
                                            "POSITION.OPPOSITE_BLOCK",
                                            {"symbol": sym, "open_side": open_side},
                                        )
                                except Exception:
                                    pass
                short_circuit = False
    ...
    try:
        if (
            ...
            and getattr(pipeline, "tw", None) is not None
            and getattr(request.app.state, "trap_window", None) is None
        ):
    ...
    try:
        if (
            ...
            and reason.startswith("spread_bps_too_wide")
            ...
            spread_limit_bps = float(
                ((cfg_all_s.get("guards") or {}).get("spread_bps_limit", 100.0))
            )
            emitter.emit(
                ...
                payload={
                    "spread_bps": float(spread_bps_val or 0.0),
                    "limit_bps": float(spread_limit_bps),
                },
            )
    except Exception:
        pass
    ...
    try:
        if diagnostics_local:
            ...
            existing = (
                set(obs["diagnostics"])
                if isinstance(obs.get("diagnostics"), list)
                else set()
            )
    ...
    if emitter:
        emitter.emit(
            ...
            payload={
                ...
                "reasons": (obs.get("reasons") if isinstance(obs, dict) else None)
                or [],
                "mode": mode,
                "observability": obs,
            },
...
async def posttrade_log(request: Request, payload: dict):
    try:
        ...
        if emitter:
            try:
                emitter.emit(
                    type=POSTTRADE_LOG, severity=None, code=None, payload=payload
                )
            except Exception:
                pass
        ...
        try:
            ...
            default_orders = (
                getattr(request.app.state, "session_dir", Path("logs")) / "orders.jsonl"
            )
            ...
            orders_path = str(
                (
                    getattr(request.app.state, "session_dir", Path("logs"))
                    / (Path(cfg_orders).name if cfg_orders else default_orders.name)
                )
            )
        except Exception:
            orders_path = str(
        ...
        try:
            ol: OrderLoggers | None = getattr(request.app.state, "order_loggers", None)
            em_ev: AuroraEventLogger | None = getattr(
                request.app.state, "events_emitter", None
            )
            if ol is None:
                ...
                try:
                    ...
                    if isinstance(resp, dict):
                        ...
                        rec.setdefault(
                            "status",
                            resp.get("status")
                            or (resp.get("info") or {}).get("status"),
                        )
                        rec.setdefault(
                            "filled",
                            resp.get("filled")
                            or (resp.get("info") or {}).get("executedQty"),
                        )
                        rec.setdefault(
                            "average", resp.get("average") or resp.get("avg_price")
                        )
                        rec.setdefault(
                            "id",
                            resp.get("id") or (resp.get("info") or {}).get("orderId"),
                        )
                except Exception:
                    pass
                status = str(rec.get("status", "")).lower()
                is_failed = bool(
                    rec.get("error") or rec.get("error_code") or rec.get("error_msg")
                )
                is_success = (
                    status in {"filled", "partially_filled", "partial", "closed"}
                    or (rec.get("filled") or 0) > 0
                )
                base = {
                    "ts": rec.get("ts")
                    or rec.get("ts_server")
                    ...
                    base.update(
                        {
                            ...
                            "avg_price": rec.get("average") or rec.get("avg_price"),
                            "fees": rec.get("fee") or rec.get("fees"),
                        }
                    )
                    ol.log_success(**base)
                    ...
                    try:
                        ...
                        try:
                            ...
                            if isinstance(resp2, dict):
                                close_flag = close_flag or bool(
                                    resp2.get("close") or resp2.get("reduceOnly")
                                )
                        except Exception:
                            close_flag = False
        ...
        try:
            ...
            try:
                if isinstance(payload, dict):
                    ...
                    if isinstance(resp, dict):
                        close_flag = close_flag or bool(
                            resp.get("close") or resp.get("reduceOnly")
                        )
            except Exception:
                close_flag = False
            ...
            if sym and cd_ms > 0 and (action == "close" or close_flag):
                ...
                em_ev2: AuroraEventLogger | None = getattr(
                    request.app.state, "events_emitter", None
                )
                if em_ev2:
                    try:
                        em_ev2.emit(
                            "AURORA.COOL_OFF",
                            {"symbol": sym, "until_ms": now_ms + cd_ms},
                            src="api",
                        )
                    except Exception:
                        pass
...
if __name__ == "__main__":
    host = (
        ((app.state.cfg or {}).get("api", {}) or {}).get("host", "0.0.0.0")
        if hasattr(app.state, "cfg")
        else "0.0.0.0"
    )
    port = (
        ((app.state.cfg or {}).get("api", {}) or {}).get("port", 8000)
        if hasattr(app.state, "cfg")
        else 8000
    )
...
async def _sli_update_loop(app: FastAPI):
    while True:
        ...
        try:
            ev_path = (
                getattr(app.state, "session_dir", Path("logs")) / "aurora_events.jsonl"
            )
            ...
            for ln in reversed(lines[-5000:]):
                try:
                    ...
                    continue
                code = str(
                    ev.get("event_code") or ev.get("code") or ev.get("type") or ""
                ).upper()
...
async def ops_cooloff(sec: int, _: bool = Depends(_ops_auth)):
    ...
    if emitter:
        emitter.emit(
            type="AURORA.COOL_OFF",
            severity="info",
            code="AURORA.COOL_OFF",
            payload={"until": until},
        )
...
async def ops_arm_disarm(mode: str, _: bool = Depends(_ops_auth)):
    ...
    if emitter:
        emitter.emit(
            type="AURORA.ARM_STATE",
            severity="info",
            code="AURORA.ARM_STATE",
            payload={"state": hg.snapshot()},
        )
    return {"status": "ok", "state": hg.snapshot()}
...
async def ops_rotate_token(body: RotateTokenRequest, _: bool = Depends(_ops_auth)):
    ...
    if emitter:
        ...
        emitter.emit(
            type="OPS.TOKEN_ROTATE",
            severity="info",
            code="OPS.TOKEN_ROTATE",
            payload={"ts": VERSION, "token_mask": mask},
        )
    return {"status": "ok"}
