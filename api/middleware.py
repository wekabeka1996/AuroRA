...
class AuthMiddleware:
    def __init__(self, app, token_getter: Callable[[], str | None] | None = None):
        ...
        self.auth_token = (token_getter() if token_getter else None) or os.getenv(
            "AURORA_API_TOKEN"
        )
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if mutating:
            ...
            if not self.auth_token:
                return JSONResponse(
                    {"detail": "AURORA_API_TOKEN not configured"}, status_code=503
                )
            if not x_auth:
...
class RateLimitMiddleware:
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if b is None:
            b = self.ip_buckets[ip] = LeakyBucket(
                capacity=self.general_rps, leak_rate=self.general_rps
            )
        if not b.allow():
        ...
        if method in {"POST", "PUT", "PATCH", "DELETE"}:
            ...
            if mb is None:
                mb = self.mutating_buckets[ip] = LeakyBucket(
                    capacity=self.mutating_rps, leak_rate=self.mutating_rps
                )
            if not mb.allow():
                return JSONResponse({"detail": "Too Many Requests"}, status_code=429)
        return await call_next(request)
