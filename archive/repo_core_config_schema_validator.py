...
class SchemaValidator:
    ...
    def _validate_object(self, value: Mapping[str, Any], schema: Mapping[str, Any], *, path: str, ctx: _Ctx, apply_defaults: bool) -> Dict[str, Any]:
        ...
        out: Dict[str, Any] = dict(value) if not apply_defaults else {}
        ...
        for k, subschema in props.items():
            if k in value:
                norm = self._validate_node(value[k], subschema, path=_path_join(path, k), ctx=ctx, apply_defaults=apply_defaults)
                if apply_defaults:
                    out[k] = norm
            elif apply_defaults and isinstance(subschema, Mapping) and "default" in subschema:
        ...
        return out if apply_defaults else dict(value)


# -------- Convenience function --------

def validate(data: _JSON, schema: Mapping[str, Any], *, apply_defaults: bool = False, allow_additional_properties: bool = True) -> _JSON:
    return SchemaValidator(schema, allow_additional_properties=allow_additional_properties).validate(data, apply_defaults=apply_defaults)
