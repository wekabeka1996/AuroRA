...
@dataclass
class DriftMonitor:
    cusum: CUSUMDetector = CUSUMDetector(k=0.25, h=6.0, clip=6.0)
    glr: GLRDetector = GLRDetector(window=200, thr=25.0, clip=6.0)
...
def _test_no_false_alarm_on_stationary() -> None:
    xs = _make_logit_series(dmu=0.0)  # no shift
    dm = DriftMonitor(cusum=CUSUMDetector(k=0.25, h=7.0, clip=6.0), glr=GLRDetector(window=150, thr=28.0, clip=6.0))
    tripped = False
    for i, x in enumerate(xs):
        out = dm.update(x, ts=float(i))
