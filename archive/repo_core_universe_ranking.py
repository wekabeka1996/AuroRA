...
class UniverseRanker:
    def __init__(
        ...
        *,
        ...
        tot = float(wL) + float(wS) + float(wP) + float(wR)
        self.wL = float(wL) / tot
        self.wS = float(wS) / tot
        self.wP = float(wP) / tot
        self.wR = float(wR) / tot
        self.addT = float(add_thresh)
        self.dropT = float(drop_thresh)
    ...
    def update_metrics(self, symbol: str, *, liquidity: float, spread_bps: float, p_fill: float, regime_flag: float) -> None:
        self._metrics[symbol] = SymbolMetrics(float(liquidity), float(spread_bps), max(0.0, min(1.0, float(p_fill))), max(0.0, min(1.0, float(regime_flag))))

    # ---------- scoring ----------

    def _robust_scale(self, vals: List[float], invert: bool = False) -> Dict[str, float]:
...
__all__ = ["UniverseRanker", "SymbolMetrics", "Ran