...
- w: weights over feature vector x (features already preprocessed/scaled)
- b: intercept (bias)
- gamma: cross-asset coupling coefficient
- beta_{i|SOL}: lead-lag beta (estimated in leadlag_hy.py)
- r_SOL(t - tau*): lagged return of SOL at tau* (anti-look-ahead respected by upstream)
...
class ScoreModel:
    ...
    def predict_proba(self, features: Mapping[str, Any], *, cross_beta: Optional[float] = None, cross_return: Optional[float] = None, calibrator: Optional[object] = None) -> float:
