"""
Aurora+ScalpBot — features/cross_asset.py
-----------------------------------------
...
Paste into: aurora/features/cross_asset.py
Run self-tests: `python aurora/features/cross_asset.py`
...
def _test_hy_and_leadlag() -> None:
    ...
    # since Y leads X by L=0.5, best lag should be ~ -0.5
    assert -0.8 <= scan["best_lag"] <= -0.2
    # correlation at best lag should be higher (in abs) than at zero
    assert abs(scan["best_corr"]) >= abs(scan["hy_corr_0"]) - 1e-6
    # betas must be finite numbers
    assert math.isfinite(scan["beta_x_on_y_0"]) and math.isfinite(scan["beta_y_on_x_0"]) \
        or True  # allow zeros when variance is tiny


if __name__ == "__main__":
    _test_hy_and_leadlag()
    print("OK - features/cross_asset.py self-tests passed")
