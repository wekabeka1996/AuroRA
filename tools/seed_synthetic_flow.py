            ...
            try:
                ...
                logger.emit(
                    "SLA.CHECK",
                    {
                        ...
                        "edge_after_bps": float(intent.expected_return_bps)
                        - 0.01 * measured_latency_ms,
                        "intent_id": intent.intent_id,
                    },
                )
                # Actual deny
                ...
                logger.emit(
                    "HEALTH.ERROR",
                    {"msg": "seed sla_deny emit error", "scenario": scenario},
                )
            continue
        ...
        if HAVE_EXECUTION and svc is not None and market is not None:
            try:
                svc.place(
                    intent,
                    market,
                    features={"pred_latency_ms": measured_latency_ms},
                    measured_latency_ms=measured_latency_ms,
                )
            except Exception:
                logger.emit(
                    "HEALTH.ERROR", {"msg": "seed scenario error", "scenario": scenario}
                )
        else:
            ...
            try:
                ...
                if scenario in ("maker", "taker"):
                    ...
                    logger.emit(
                        "ORDER.PLAN.BUILD",
                        {"intent_id": intent.intent_id, "tca": {"net_after_tca": 5}},
                    )
                    if scenario == "maker":
                        logger.emit(
                            "ROUTER.DECISION",
                    ...
                    logger.emit(
                        "ORDER.DENY",
                        {"intent_id": intent.intent_id, "reason": "LOW_PFILL.DENY"},
                    )
                elif scenario == "size_zero":
                    logger.emit(
                        "ORDER.DENY",
                        {"intent_id": intent.intent_id, "reason": "SIZE_ZERO.DENY"},
                    )
                ...
                logger.emit(
                    "HEALTH.ERROR",
                    {"msg": "seed fallback emit error", "scenario": scenario},
                )
    # restore time
    time.time = orig_time
...
def _parse_args():
    ...
    p.add_argument(
        "--scenarios", type=str, default="maker,taker,low_pfill,size_zero,sla_deny"
    )
    p.add_argument("--n", type=int, default=1)
    p.add_argument(
        "--truncate",
        action="store_true",
        help="Remove output file if it exists before seeding",
    )
    return p.parse_args()
