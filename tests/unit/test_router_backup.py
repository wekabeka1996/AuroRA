...
from core.execution.router_backup import (
    Router, QuoteSnapshot, RouteDecision
)
...
class TestRouterInitialization:
    ...
    def test_init_with_custom_params(self, mock_get_config):
        ...
        custom_sla = SLAGate(max_latency_ms=50.0, kappa_bps_per_ms=0.1, min_edge_after_bps=0.5)
        custom_hazard = CoxPH()
...
class TestRouterDecisionLogic:
    ...
    def test_decide_sla_deny_fallback_maker(self):
        ...
        strict_sla = SLAGate(max_latency_ms=1.0, kappa_bps_per_ms=10.0, min_edge_after_bps=5.0)
        router = Router(
            slagate=strict_sla,
            min_p_fill=0.3,
            fees=self.fees
...
class TestRouterInternalMethods:
    ...
    def test_tca_net_edge_bps_maker(self):
        ...
        net_edge = self.router._tca_net_edge_bps(
            "maker", features, 5.0, 10.0, 0.1
        )
...
class TestRouterLogging:
    ...
    def test_decide_creates_log_entry(self):
        ...
        assert decision.route == "deny"
        assert "unattractive" in decision.reason.lower() or "denied" in decision.reason.lower()
